...
export function analyseModel(model: BuildingModel, rules: RuleDefinition[] = builtinRules, locale: Locale = "en"): Finding[] {
  ...
  return model.doors.flatMap((item) => active.map((rule) => {
    const finding = rule.field === "clearWidth" ? widthFinding(item, rule, locale) : informationFinding(item, rule, locale);
    return { ...finding, priority: finding.status === "FAIL" ? (rule.severity === "HIGH" ? "P1" : "P2") : finding.status === "REVIEW" ? (rule.severity === "HIGH" ? "P2" : "P3") : undefined };
  }));
}
...
function buildReportBase(model: BuildingModel, findings: Finding[], locale: Locale): string {
  ...
  const renderFinding = (item: Finding, index: number) => `### ${index + 1}. ${item.priority ? `${item.priority} · ` : ""}${status(item.status)} — ${item.elementName}\n\n**${zh ? "审查结论" : "Review outcome"}:** ${item.message}\n\n- **${zh ? "机器状态代码" : "Machine status"}:** \`${item.status}\`\n- **${zh ? "整改优先级" : "Remediation priority"}:** ${item.priority ?? (zh ? "无需整改" : "No remediation required")}\n- **${zh ? "适用规则" : "Applied rule"}:** ${item.ruleTitle} (${item.ruleId} v${item.ruleVersion})\n- **${zh ? "构件标识" : "Element GlobalId"}:** \`${item.elementId}\`\n- **${zh ? "观测证据" : "Observed evidence"}:** ${item.observed}\n- **${zh ? "验收准则" : "Acceptance criterion"}:** ${item.required}\n- **${zh ? "证据路径" : "Evidence path"}:** \`${item.evidencePath}\`\n- **${zh ? "证据质量" : "Evidence quality"}:** ${reliability(item.reliability)}\n- **${zh ? "整改或跟进行动" : "Corrective or follow-up action"}:** ${item.nextStep}`;
  const failures = findings.filter((item) => item.status === "FAIL");
  const reviews = findings.filter((item) => item.status === "REVIEW");
  const closed = findings.filter((item) => item.status === "PASS" || item.status === "NOT_APPLICABLE");
  const prioritySummary = ["P1", "P2", "P3"].map((priority) => `${priority}: ${findings.filter((item) => item.priority === priority).length}`).join(zh ? "；" : "; ");
  const empty = (text: string) => `_${text}_`;
  return `# ${zh ? "BIM 模型合规预审报告" : "BIM Model Compliance Pre-review Report"}\n\n## ${zh ? "模型名称" : "Model name"}\n\n**${model.name}**\n\n- **${zh ? "IFC 架构" : "IFC schema"}:** ${model.schema}\n- **${zh ? "模型单位" : "Model units"}:** ${zh ? ({ mm:"毫米", m:"米", unresolved:"未确定" } as const)[model.units] : model.units}\n- **${zh ? "报告生成时间" : "Generated"}:** ${new Date().toISOString()}\n\n## ${zh ? "审查结论" : "Review conclusion"}\n\n${conclusion}\n\n${zh ? `共执行 ${findings.length} 项规则结果：${counts.FAIL} 项已确认不符合，${counts.REVIEW} 项待专业复核，${counts.PASS} 项通过，${counts.NOT_APPLICABLE} 项不适用。` : `${findings.length} rule results were recorded: ${counts.FAIL} confirmed non-compliances, ${counts.REVIEW} matters requiring professional review, ${counts.PASS} passes and ${counts.NOT_APPLICABLE} not-applicable results.`}\n\n## ${zh ? "违规明细" : "Confirmed non-compliance details"}\n\n${failures.length ? failures.map(renderFinding).join("\n\n") : empty(zh ? "未发现已确认的违规项。待复核事项不计作已确认违规。" : "No confirmed non-compliances were identified. Review items are not treated as confirmed failures.")}\n\n## ${zh ? "待专业复核事项" : "Matters requiring professional review"}\n\n${reviews.length ? reviews.map(renderFinding).join("\n\n") : empty(zh ? "没有待复核事项。" : "No matters require professional review.")}\n\n## ${zh ? "整改优先级建议" : "Remediation priority recommendations"}\n\n${prioritySummary}\n\n- **P1:** ${zh ? "已确认的高严重度不符合项；应立即处理并重新审查。" : "Confirmed high-severity non-compliance; address immediately and re-review."}\n- **P2:** ${zh ? "高严重度证据缺口或一般不符合项；应在下一次设计签发前处理。" : "High-severity evidence gap or ordinary non-compliance; resolve before the next design issue."}\n- **P3:** ${zh ? "一般证据完整性事项；纳入正常信息完善流程。" : "Ordinary evidence-completeness matter; resolve through the normal information-enrichment workflow."}\n\n## ${zh ? "通过及不适用项目摘要" : "Passed and not-applicable summary"}\n\n${closed.length ? `${counts.PASS} ${zh ? "项通过；" : "passed; "}${counts.NOT_APPLICABLE} ${zh ? "项不适用。" : "were not applicable."}` : empty(zh ? "所选范围没有通过或不适用项目。" : "The selected scope contains no passed or not-applicable results.")}\n\n---\n${zh ? "限制声明：本报告仅用于专业预审，不构成法定认证，也不能替代合资格专业人士的判断。" : "Limitation: this report supports professional pre-review only. It does not certify statutory compliance or replace the judgement of a suitably qualified professional."}\n`;
}
