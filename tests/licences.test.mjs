...
import { parseIfc } from "../lib/compliance.ts";
...
test("the concise manual IFC fixture is licensed, exact and structurally readable", async () => {
  const manifest = JSON.parse(await readFile(new URL("../manual-test-files/manifest.json", import.meta.url), "utf8"));
  const bytes = await readFile(new URL(`../manual-test-files/${manifest.ifc.filename}`, import.meta.url)); const hash = createHash("sha256").update(bytes).digest("hex");
...
test("the official HKSAR manual fixture is explicitly local-only and Git-ignored", async () => {
  const manifest = JSON.parse(await readFile(new URL("../manual-test-files/manifest.json", import.meta.url), "utf8")); const ignore = await readFile(new URL("../.gitignore", import.meta.url), "utf8");
  assert.equal(manifest.official_local_reference.redistribution, false); assert.match(manifest.official_local_reference.source, /^https:\/\/www\.bd\.gov\.hk\//); assert.match(ignore, /manual-test-files\/hk-official\/\*\.pdf/);
  try { const bytes = await readFile(new URL(`../manual-test-files/${manifest.official_local_reference.filename}`, import.meta.url)); assert.equal(createHash("sha256").update(bytes).digest("hex"), manifest.official_local_reference.sha256); assert.match(bytes.subarray(0, 8).toString(), /^%PDF-/); } catch (error) { if (error?.code !== "ENOENT") throw error; }
  try { const bytes = await readFile(new URL(`../manual-test-files/${manifest.official_local_quick_fixture.filename}`, import.meta.url)); assert.equal(createHash("sha256").update(bytes).digest("hex"), manifest.official_local_quick_fixture.sha256); assert.match(bytes.subarray(0, 8).toString(), /^%PDF-/); } catch (error) { if (error?.code !== "ENOENT") throw error; }
});
