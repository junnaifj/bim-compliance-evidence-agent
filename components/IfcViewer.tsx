...
export default function IfcViewer({ source, sourceKey, findings, selectedGlobalId, onSelect, locale }: Props) {
    ...
    async function start() {
      ...
      try {
        ...
        controlsRef.current = {
          ...
          setSelected: (globalId) => { visual.selectedGlobalId = globalId; visual.hoveredGlobalId = undefined; setInternalSelectedId(globalId); setHovered(null); applyVisual(); if (globalId) { const objects = byGlobalId.get(globalId); if (objects?.length) { const box = new THREE.Box3(); objects.forEach((object) => box.expandByObject(object)); fit(box.expandByScalar(radius * 0.02)); } } },
        ...
        applyVisual(); setState("ready"); setMessage(locale === "zh" ? `${root.children.length} 个几何片段 · 悬停预览；同一点重复单击可选择内部构件` : `${root.children.length} geometry fragments · hover to preview; click repeatedly to select internal elements`);
  ...
  return <div className="ifc-viewer">
    ...
    {hovered && !internalSelectedId && <div className="element-tooltip" style={{ left: tooltip.x, top: tooltip.y }}><strong>{hovered.name}</strong><span>{hovered.entityType}</span><code>{hovered.globalId}</code><small>{hoverFinding ? hoverFinding.status.replace("_", " ") : (locale === "zh" ? "无适用的启用规则" : "No applicable active rule")}</small>{diagnostics.pickDepth > 1 && <small>{locale === "zh" ? `${diagnostics.pickDepth} 个重叠构件 · 重复单击选择后方构件` : `${diagnostics.pickDepth} overlapping elements · click again for the element behind`}</small>}</div>}
